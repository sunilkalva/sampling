...
RAW = LOAD '$input' USING com.inmobi.dw.yoda.loader.PigLoader('rq_siteid,cl_id')
AS (
  siteid : chararray,
  clid : chararray
);

A = filter RAW by UPPER(clid) != 'NULL';

STORE A INTO '/output/siteclid' using PigStorage(',');

a_count = foreach(group A by siteid) {generate group AS siteid, COUNT(A);};

STORE a_count INTO '$output' using PigStorage(',');